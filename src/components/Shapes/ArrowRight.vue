<template>
  <svg
    ...
    style="enable-background:new 0 0 511.633 511.633;"
    xml:space="preserve"
  >
    <g fill="#cc7a00">
      <g>
        <path
          d="M506.206,179.012L360.025,32.834c-3.617-3.617-7.898-5.426-12.847-5.426s-9.233,1.809-12.847,5.426
    ...
    <g @mouseover="hoverText" ref="texts" v-for="(text, index) in texts" :key="text">
      <g
        class="text"
        ref="text"
        stroke="#fffff0"
        stroke-width="0.2"
        :transform="`scale(2.2, 2.2) translate(78, ${48 * parseInt(index) + 90})`"
      >
        <text>{{ text }}</text>
      </g>
    </g>
...
export default {
  ...
  methods: {
    hoverText: function() {
      const textsRef = this.$refs["texts"];
      this.$anime({
        targets: textsRef,
        rotate: [-15, 25, 0],
